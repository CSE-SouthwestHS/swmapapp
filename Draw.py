...
def draw_path(vertices):
    ...
    if len(vertices) == 1:
        #one item case
        draw_vertex(curdraw, vertices[0], 25)
    ...
    draw_vertex(curdraw, vertices[0])
    ix = 0
    while ix < len(vertices)-1:
        v1 = vertices[ix]
        v2 = vertices[ix + 1]
        if v1.getfloor() == v2.getfloor():
            draw_line(curdraw, v1, v2, 25)
        else:
            draw_vertex(curdraw, v1)
            name = pathfolder + v1.getfloor() + ".png"
            curimage.save(name)
            curimage = Image.open(floor_map[v2.getfloor()])
            curdraw = ImageDraw.Draw(curimage)
            draw_vertex(curdraw, v2)
        ix += 1
    draw_vertex(curdraw, vertices[-1])
